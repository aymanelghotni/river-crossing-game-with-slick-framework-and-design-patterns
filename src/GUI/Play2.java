...
public class Play2 extends BasicGameState{
	...
	Command doMove;
	Image close;
	Image instructionsBtn;
	Image instructionsBoard;
	Image world;
	Image moveButton;
	Image label;
	Image boatImageLeft;
...
public Play2(int state) {
	...
	public void init(GameContainer gc, StateBasedGame sbg) throws SlickException{
		 ...
		 instructionsBtn=new Image("data/instructions.png");
		 instructionsBoard=new Image("data/instructionsBoard.png");
		...
		newGame=new newGameCommand(level,game);
		newGame.execute();
		
		 BufferedImage[] farmer90Images= game.getCrossersOnLeftBank().get(0).getImages();
		 BufferedImage[] farmer80Images=game.getCrossersOnLeftBank().get(1).getImages();
		 BufferedImage[] farmer60Images=game.getCrossersOnLeftBank().get(2).getImages();
		 BufferedImage[] farmer40Images=game.getCrossersOnLeftBank().get(3).getImages();
		 BufferedImage[] animal20Images=game.getCrossersOnLeftBank().get(4).getImages();
		...
		animal20=movingRightAnimal20;
		 farmer90Obj=game.getCrossersOnLeftBank().get(0);
		 farmer80Obj=game.getCrossersOnLeftBank().get(1);
		 farmer40Obj=game.getCrossersOnLeftBank().get(3);
		 farmer60Obj=game.getCrossersOnLeftBank().get(2);
		 animal20Obj=game.getCrossersOnLeftBank().get(4);
	...
	public void render(GameContainer gc, StateBasedGame sbg, Graphics g) throws SlickException{
		...
		instructionsBtn.draw(600, 30, 0.1f);
		...
		if(flagInst==1)
		{
			instructionsBoard.draw();
			int i=0;
			String s=game.getInstructions()[0]+"\n";
			for(i=1;i<game.getInstructions().length;i++)
			{
				s+=(game.getInstructions()[i]+"\n");
			}
			g.drawString(s, 70, 200);
		}
	...
	public void update(GameContainer gc, StateBasedGame sbg, int delta) throws SlickException{
		...
		if(flagWin==0)
		{
			if(xpos>605 && xpos<670 && ypos>500 && ypos<565 && input.isMouseButtonDown(0))
			{
				flagInst=1;
			}
			...
			if(flagInst==1 && input.isMousePressed(0))
			{
				flagInst=0;
			}
